...
class MaskedData(Data):
    def get_test_data(self, forecast_date: pd.Timestamp) -> tuple[pd.DataFrame, pd.Series]:
        """For horizon=1, take sales_date = forecast_date + Timedelta(days=1), and horizon=1"""
    ...
    def _mask_features(self, data: pd.DataFrame, horizon: int) -> pd.DataFrame:
        features_to_mask = self.features.get_unavailable_features(horizon=horizon)
        data[features_to_mask] = np.nan
        return data
